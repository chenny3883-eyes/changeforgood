...
The "Meet the Founder" bio section uses a two-column flex layout:
...
**Right inset panel — stacked vertically with 14px gap:**

1. **Photo card** — `chenny-warm.jpeg` displayed in a rounded card (`border-radius: 12px`, `overflow: hidden`, `box-shadow: 0 4px 24px rgba(0,0,0,0.35)`). `object-position: center 20%` to frame Chenny's face and upper body.

2. **Logo card** — CFG logo (`cfg-logo.png`) displayed in a white rounded card (`background: rgba(255,255,255,0.94)`, `border-radius: 10px`, `padding: 12px 16px`, `box-shadow: 0 2px 12px rgba(0,0,0,0.18)`). Max width 180px, centered.

**Bio text (six paragraphs — to appear in the left column):**
...
The credentials section uses a two-sided chronological timeline. A vertical gold spine runs down the centre of the section. Entries alternate left and right of the spine, with the newest credential at the top and oldest at the bottom.

**Visual design:**
- **Spine:** 1px gold line (`rgba(212,160,23,0.3)`) centred on the wrapper
- **Key milestone dot:** Solid gold (`#D4A017`) with gold glow — used for major roles and key certifications
- **Standard dot:** Deep purple fill (`#1a1560`) with gold border — used for supporting credentials
- **Category label:** 9px, uppercase, gold, letter-spaced
- **Credential name:** 12px, weight 600, white
- **Institution:** 11px, muted white
- **Year:** 10px, gold (70% opacity), weight 600

**Legend (above timeline):**
- Gold dot = Roles & leadership
- Empty dot = Education & certifications
...
**Section background:** The Credentials and Experience section uses a CFG-branded gradient background (`.tl-section-bg` CSS class) to visually distinguish it from the surrounding sections:

```css
.tl-section-bg {
  background:
    radial-gradient(ellipse 80% 40% at 50% 0%, rgba(212,160,23,0.10) 0%, transparent 65%),
    radial-gradient(ellipse 60% 30% at 20% 100%, rgba(83,74,183,0.18) 0%, transparent 60%),
    radial-gradient(ellipse 60% 30% at 80% 100%, rgba(212,160,23,0.07) 0%, transparent 60%),
    linear-gradient(160deg, #1a1560 0%, #2D2380 40%, #1e1870 70%, #1a1560 100%);
  border-top: 1px solid rgba(212,160,23,0.15);
  border-bottom: 1px solid rgba(212,160,23,0.15);
}
```

- Soft gold radial glow at the top edge draws the eye to the section header
- Deep-purple-to-royal-purple diagonal base uses both core CFG brand colours
- Thin gold top and bottom borders (15% opacity) frame the section cleanly against adjacent sections

**Mobile behaviour:** On screens ≤420px the two-sided layout collapses to a single left-aligned column with the spine on the left edge.

**CSS implementation:**
- `.tl-wrap` — relative container; holds spine and all rows
- `.tl-spine` — absolute centred 1px gold line
- `.tl-row` — flex row; three children: `.tl-left` (flex:1, text-right), `.tl-node` (flex:0 0 0px), `.tl-right` (flex:1, text-left); `.tl-spacer` fills the empty side when a row has only one entry
- `.tl-dot` / `.tl-dot.tl-key` — absolutely positioned dot on the node
...
The Signature Programme featured card and the five-card horizontal scrolling carousel are wrapped together inside a single container with a shared `lake.jpg` photo background — the same treatment used for the quote + What We Do section on the Home page.

This makes the Signature Programme and the carousel feel visually unified as one immersive band, reinforcing the premium feel of the programme.

**Background:** Uses the `.quote-photo-section` CSS class:
```css
background:
  linear-gradient(to bottom,
    rgba(26, 21, 96, 0.62) 0%,
    rgba(45, 35, 128, 0.48) 50%,
    rgba(26, 21, 96, 0.68) 100%),
  url('/photos/lake-copy.jpg') center 40% / cover no-repeat;
```

The `.section` background inside the wrapper is overridden to `transparent` so the photo shows through.
...
The five remaining services are displayed in a BCG-style horizontal scrolling carousel (same behaviour as the Home carousel). Cards use dark purple gradient backgrounds with rgba opacity 0.88 so the lake background shows through the gaps between cards.
...
**Note:** The middle stat was updated from "strategic initiatives for PAL" to "strategic initiatives for networked teams" to better reflect the breadth of work across multiple engagements.
...
**HTML structure:** The photo background is applied using a wrapper div pattern — `.teacher-bg` is the outer wrapper carrying the photo, and `.section.section-alt` is the inner child. This ensures the photo background is not overridden by the section's default background colour.
...
**Timeline-specific colours (About page credentials section):**

| Element | Colour |
|---|---|
| Timeline spine | rgba(212,160,23,0.3) — gold at 30% opacity |
| Key milestone dot | #D4A017 (solid gold) with `box-shadow: 0 0 8px rgba(212,160,23,0.5)` |
| Standard dot | #1a1560 (deep purple) with `border: 2px solid rgba(212,160,23,0.4)` |
| Category label | var(--cfg-gold) at 85% opacity |
| Credential name | #ffffff |
| Institution | rgba(255,255,255,0.50) |
| Year | rgba(212,160,23,0.7) |
...
| Timeline credential name | 12px, weight 600, white |
| Timeline category | 9px, weight 700, uppercase, letter-spacing 0.09em, gold |
| Timeline year | 10px, weight 600, gold at 70% opacity |
...
| Timeline dot (standard) | 10×10px circle, deep purple fill, gold border |
| Timeline dot (key) | 10×10px circle, solid gold fill, gold glow shadow |
| Photo inset card (About bio) | border-radius: 12px, box-shadow: 0 4px 24px rgba(0,0,0,0.35) |
| Logo card (About bio) | White background rgba(255,255,255,0.94), border-radius: 10px, padding: 12px 16px |
...
| Services page — Signature Programme + carousel wrapper | `lake-copy.jpg` via `.quote-photo-section` class — the same photo background wraps both the featured card and the carousel. |
| Services hero right panel | `chenny-photo3.jpeg` — Chenny at a whiteboard, facilitating a session. Real photo, not stock. |
| About bio text — right inset | `chenny-warm.jpeg` — Chenny in warm, professional, natural light. Displayed at 220px wide alongside the bio text. Object-position: center 20% to frame face and upper body. |
...
**Medium overlay (Lake — quote and What We Do wrapper; Services Signature Programme wrapper):**
```css
linear-gradient(to bottom,
  rgba(26, 21, 96, 0.62) 0%,
  rgba(45, 35, 128, 0.48) 50%,
...
**Photo background wrapper pattern (used by teacher-bg, quote-photo-section):**

When a section with a default background colour needs a photo background applied, use a wrapper div to carry the photo — do NOT apply the photo directly to the `.section` or `.section-alt` element. The inner `.section` background is overridden to `transparent !important` via the wrapper's CSS class.

```css
/* Wrapper carries the photo */
.teacher-bg {
  background: [gradient], url('/photos/teacher2a.png') ...;
}
/* Inner section made transparent */
.teacher-bg .section-alt {
...
| `lake-copy.jpg` | Signature Programme card (Home carousel + Services featured card + Services wrapper) | Services |
| `chenny-photo3.jpeg` | Services hero right panel | Services |
| `cfg-logo.png` | Home hero right panel (white card); About bio right panel (logo card) | Home |
| `teacher2a.png` | Results — What Clients Say section | Results |
| `chenny-warm.jpeg` | About — bio text right inset | Photos/Home |
